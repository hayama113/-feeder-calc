...
function nav(name){
  ...
  if(name==='tyres'){
    renderTyreSelectors(state);
    updateTyreNextButton();
  }
  if(name==='weather') renderWeather();
  if(name==='final') renderFinal();
}
function updateTyreNextButton(){
  const btn=document.getElementById('to-qualifying');
  if(!btn) return;
  const hasGrid=!!(state.qualifying && state.qualifying.grid && state.qualifying.grid.length);
  btn.textContent = hasGrid ? (lang()==='ja'?'天候確認へ':'Weather Check') : (lang()==='ja'?'予選開始':'Start Qualifying');
}
...
function wire(){
  ...
  document.getElementById('to-qualifying').onclick=()=>{
    saveSetup();
    if(state.qualifying && state.qualifying.grid && state.qualifying.grid.length){
      nav('weather');
    } else {
      nav('qualifying');
      prepareQualifying();
