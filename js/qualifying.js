...
export function startQualifying(state, onDone){
  ...
  area.innerHTML = '';
  let score=0, combo=0, time=15.0, active=null, running=true;
  const diff=state.setup.difficulty;
  const cfg = diff==='B'
    ? {size:.105, interval:820, feint:0, penalty:0, cls:'diff-b'}
    : diff==='A'
      ? {size:.078, interval:680, feint:.10, penalty:1, cls:'diff-a'}
      : {size:.060, interval:540, feint:.20, penalty:2, cls:'diff-s'};
  function place(){
    if(!running) return;
    if(active) active.remove();
    const isFeint = Math.random() < cfg.feint;
    const rect = area.getBoundingClientRect();
    const size = Math.max(diff==='S'?36:42, rect.width*cfg.size);
    ...
    dot.addEventListener('pointerdown', ev=>{
      ...
      scoreEl.textContent=score; comboEl.textContent=combo; place();
    });
    area.appendChild(dot); active=dot;
  }
  area.addEventListener('pointerdown', function miss(e){
    if(!running) { area.removeEventListener('pointerdown',miss); return; }
    if(e.target===area){ score-=1; combo=0; scoreEl.textContent=score; comboEl.textContent=combo; beep(state,140,90,'sawtooth'); }
  });
  place();
  const timer = setInterval(()=>{
    time-=0.1; timeEl.textContent=Math.max(0,time).toFixed(1);
    if(time<=0){
      clearInterval(timer); running=false; if(active) active.remove();
      const grid = buildGrid(state, score); state.qualifying = { score, grid }; onDone(grid, score);
    }
  },100);
}
